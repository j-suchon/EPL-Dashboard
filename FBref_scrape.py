...
def get_league_table():
    html_content = requests.get(
        'https://fbref.com/en/comps/9/Premier-League-Stats').text.replace('<!--', '').replace('-->', '')
...
def get_fixtures():
    html_content = requests.get(
        'https://fbref.com/en/comps/9/schedule/Premier-League-Scores-and-Fixtures').text.replace('<!--', '').replace('-->', '')
    df = pd.read_html(html_content)
    return df[0]


# HERE WE ESTABLISH THE VARIABLES FOUND ON FBREF TO BE CALLED AND
# ORGANIZED LATER
league = [
    'rank',
    'Squad',
...
def get_tables(url):
    ...
    # The next two lines get around the issue with comments breaking the
    # parsing.
    comm = re.compile("<!--|-->")
...
def get_frame_team(features, team_table):
    ...
    for row in rows_squad:
        if(row.find('th', {"scope": "row"}) is not None):
            name = row.find('th', {"data-stat": "squad"}
...
# passing, passing types, goal and shot creation, defensive actions,
# possession, etc...
...
def player_imgs(eplsquad):
    ...
    for team in teams_soup:
        team_dict.update(
            {team.text: F"https://www.premierleague.com{links[x]['href'].replace('overview', 'squad')}"})
        x += 1
...
about = """
               This application was built entirely using python and Streamlit with
               football reference data (https://fbref.com). Special thanks to those at 
               FBref and Streamlit who do a great job maintaining these tools.
               
               ---
               
               This app is a dashboard that runs an analysis on any desired EPL player or squad. 
               six visualizations of various statistics are displayed
               for the selected player. They are briefly described below: \n
               
               ### EPL Page ### \n
               
               **Current League Table** - A snapshot of the current league table. The champions league,
                    europa league, and relegation spots are highlighted in green, blue, and red, respectively.\n
               
               **Top 5 Stat League Leaders** - A display of the top 5 players for each stat in the dropdown and
               their headshot sourced from http://www.premierleague.com. \n
               Stats include:         
               Goals, Assists, Key Passes, Passes into final 3rd, Goal-creating Actions, Shot-creating Actions, 
               Tackles Won, Pressures, Blocked Shots, Interceptions, Dribbles Completed, Progressive Carries, 
               Carries into final 3rd, Aerial Duel Win % \n
               
               **Week by Week** - A table corresponding to the selected matchweek in the dropdown. \n
               
               ### Team Page ### \n
               
               **Previous Fixture Table** - A table combining all previous fixtures for selected team.
                The team is highlighted with the primary team color. Table also displays xG and score for selected
                team and opponent.\n
               ...
               **Next Fixture** - Compares the upcoming opponent to the selected team. Categories: 
                Goals per 90, xG per 90, Goals against per 90, Clean Sheets, Shots on Target per 90, 
                Shot-Creating Actions per 90, Goal-Creating Actions per 90, Pressures in Defensive 3rd,
                Pressures in Middle 3rd, Pressures in Attacking 3rd. *The dominant team stat is highlighted
                 in green and the lesser in red.* \n
               
               **Team Stats** - Displays a bar chart of the stat selected from the dropdown. Stats: 
               Goals, Assists, Key Passes, Passes into final 3rd, Goal-creating Actions, Shot-creating Actions, 
               Tackles Won, Pressures, Blocked Shots, Interceptions, Dribbles Completed, Progressive Carries, 
               Carries into final 3rd, Aerial Duel Win %\n
               
               **Tracking xG by Week** - This line chart tracks the Expected Goals statistic of the selected
               team and their opponent through each week of the current season.\n
               
               **Player Stats** - A radar/spider chart for the selected player from the dropdown box. The stats
               in the radar chart depend on the players position. The selected players stats are compared to the
               league average for that position. Information and basic statistics are also displayed to the right
               of the headshot. \n
               ...
               *The initial page load will take around 40 seconds. Since I am not storing this data, nor
               are there any public API's, this initial load is a webscrape and cleansing of all related data.
               Streamlit provides a great caching decorator that is used on all functions so beyond the first page load,
               all wait times should be minimal* \n
               
               Joshua Suchon 2021 -- source code: https://github.com/j-suchon
               """